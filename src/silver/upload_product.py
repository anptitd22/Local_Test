...
def upload_to_iceberg(df: DataFrame) -> None:
    ...
    try:
        ...
        tbl = catalog.create_table(f"{args.namespace}.product", schema=schema, partition_spec=partition_silver)
    except:
        tbl = catalog.load_table(f"{args.namespace}.product")
...
def delete_iceberg_table() -> None:
    ...
    try:
        catalog.drop_table(f"{args.namespace}.product")
        print(f"Table '{args.namespace}.product' has been deleted.")
