...
#variable
file_path = '../../dataset/datamart/Product.csv'
...
def upload_to_iceberg(df: DataFrame) -> None:
    ...
    try:
        ...
        tbl = catalog.create_table("silver.product", schema=schema, partition_spec=partition_silver)
    except:
        tbl = catalog.load_table("silver.product")
...
def delete_iceberg_table() -> None:
    ...
    try:
        catalog.drop_table("silver.product")
        print("Table 'silver.product' has been deleted.")
