...
def upload_to_iceberg(df: DataFrame) -> None:
    ...
    try:
        ...
        tbl = catalog.create_table(f"{args.namespace}.order_detail", schema=schema, partition_spec=partition_silver)
    except:
        tbl = catalog.load_table(f"{args.namespace}.order_detail")
...
def delete_iceberg_table() -> None:
    ...
    try:
        catalog.drop_table(f"{args.namespace}.order_detail")
        print(f"Table '{args.namespace}.order_detail' has been deleted.")
