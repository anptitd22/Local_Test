...
WHERE is_current = true
AND sales_order_detail_id IN (    
    SELECT s.sales_order_detail_id
    FROM (
        SELECT
            od.sales_order_id,
            od.sales_order_detail_id,
    ...
    JOIN iceberg.gold.dim_order d2
        ON d2.sales_order_detail_id = s.sales_order_detail_id
        AND d2.is_current = true
        AND d2.active_start < timestamp '{{data_interval_end}}'
    WHERE
        COALESCE(d2.sales_order_id, -1) <> COALESCE(s.sales_order_id, -1)
        OR COALESCE(d2.product_id, -1) <> COALESCE(s.product_id, -1)
        OR COALESCE(d2.customer_id, -1) <> COALESCE(s.customer_id, -1)
        OR COALESCE(d2.order_date, TIMESTAMP '1900-01-01') <> COALESCE(s.order_date, TIMESTAMP '1900-01-01')
        OR COALESCE(d2.ship_date, TIMESTAMP '1900-01-01') <> COALESCE(s.ship_date, TIMESTAMP '1900-01-01')
        OR COALESCE(d2.sales_order_number, '') <> COALESCE(s.sales_order_number, '')
);
...
WITH stg_order__source AS (
    SELECT
        ...
        od.sales_order_detail_id,
        od.product_id,
        od.updated_at AS detail_updated_at,  -- alias rõ ràng
        oh.customer_id,
        oh.order_date,
        oh.ship_date,
        oh.sales_order_number
        -- nếu cần created_at thì SELECT riêng, tránh lặp updated_at
        -- , od.created_at
    FROM iceberg.gold.stg_order_detail od
    JOIN iceberg.gold.stg_order_header oh
      ON od.sales_order_id = oh.sales_order_id 
...
record_changed_local AS (
    ...
    JOIN stg_order__source s
     ...
     AND d.active_start < timestamp '{{data_interval_start}}'
    WHERE
...
SELECT
    ABS(from_big_endian_64(
        xxhash64(
            to_utf8(
                ...
                cast(date(o.detail_updated_at) as varchar)
            )
        )
    )) AS sales_order_key,
    o.sales_order_id,
    ...
    TRUE AS is_current,
    o.detail_updated_at AS active_start,
    TIMESTAMP '9999-12-31' AS active_end
