...
AND product_id IN (    
    ...
    JOIN iceberg.gold.dim_product d2
        ON d2.product_id = s.product_id
        AND d2.is_current = true
        AND d2.active_start < timestamp '{{data_interval_end}}'
    WHERE
        COALESCE(d2.product_category_id, -1) <> COALESCE(s.product_category_id, -1)
        OR COALESCE(d2.product_sub_category_id, -1) <> COALESCE(s.product_sub_category_id, -1)
        OR COALESCE(d2.product_list_price, -1) <> COALESCE(s.list_price, -1)
        OR COALESCE(d2.product_name, '') <> COALESCE(s.name, '')
        OR COALESCE(d2.product_color, '') <> COALESCE(s.color, '')
        OR COALESCE(d2.product_size, '') <> COALESCE(s.size, '')
        OR COALESCE(d2.sub_category_name, '') <> COALESCE(s.sub_category_name, '')
        OR COALESCE(d2.category_name, '') <> COALESCE(s.category_name, '')
);
...
record_changed_global AS (
    ...
    WHERE s.product_id NOT IN (
        SELECT DISTINCT product_id FROM iceberg.gold.dim_product
    )
),
insert_records AS (
...
SELECT
    ABS(from_big_endian_64(
        xxhash64(
            to_utf8(
                cast(p.product_id as varchar) || ':' ||
                cast(date(p.product_updated_at) as varchar)
            )
        )
    )) AS product_key,
    p.product_id,
    ...
    p.category_name,
    TRUE,
    p.product_updated_at  AS active_start,
    TIMESTAMP '9999-12-31' AS active_end
